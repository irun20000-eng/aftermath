...
';
  ...
  function injectDOM() {
    ...
    wrap.innerHTML = ''
      + '<button class="aftermath-timer-toggle" id="aftermathTimerToggle" aria-label="타이머 열기/닫기" title="타이머" onclick="aftermathTimerToggle()">'
      ...
      +     '<span>⏱ 타이머</span>'
      +     '<button class="aftermath-timer-close" id="aftermathTimerClose" aria-label="닫기" onclick="aftermathTimerClose()">×</button>'
      +   '</div>'
      +   '<div class="aftermath-timer-display" id="aftermathTimerDisplay">00:00</div>'
      +   '<div class="aftermath-timer-presets">'
      +     '<button type="button" onclick="aftermathTimerPreset(60)">1분</button>'
      +     '<button type="button" onclick="aftermathTimerPreset(180)">3분</button>'
      +     '<button type="button" onclick="aftermathTimerPreset(300)">5분</button>'
      +     '<button type="button" onclick="aftermathTimerPreset(600)">10분</button>'
      +   '</div>'
      +   '<div class="aftermath-timer-controls">'
      +     '<input type="number" id="aftermathTimerInput" placeholder="초" min="1" inputmode="numeric">'
      +     '<button type="button" class="primary" id="aftermathTimerStart" onclick="aftermathTimerStart()">시작</button>'
      +     '<button type="button" id="aftermathTimerPause" onclick="aftermathTimerPause()">일시정지</button>'
      +     '<button type="button" id="aftermathTimerReset" onclick="aftermathTimerReset()">리셋</button>'
      +   '</div>'
      + '</div>';
